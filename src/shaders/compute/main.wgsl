...
fn blinkenlights(n: u32, pixel: vec2<u32>, out_color: ptr<function, vec3<f32>>) -> bool {
    ...
    let display_length = tree_length * 5;

    if (all(pixel >= vec2<u32>(6)) && all(pixel < vec2<u32>(10 + display_length, 42))) {
        if (any(pixel < vec2<u32>(8)) || any(pixel >= vec2<u32>(8 + display_length, 40))) {
...
@compute @workgroup_size(8, 8, 1) fn cs_main(
    ...
    if (blinkenlights(uniforms.debug_variable_0, pixel, &blinkenlight)) {
